  ...
  <div>
    <div class="skin-codinglife has-navbar">
      <div id="home">
        <div id="main">
          <div id="mainContent">
            <div class="forFlow" v-if="articleList && articleList">
              <div v-for="item in articleList" :key="item.id" v-show="isShow(item.tag)" class="day" role="article" aria-describedby="postlist_description_11168708">
                <div class="dayTitle">
                  <div>{{ item.createTimeFormat }}</div>
                </div>

                <div class="postTitle" role="heading" aria-level="2">
                  <a class="postTitle2 vertical-middle pinned-post" :href="'http://www.inlife.ink/detail?id=' + item.id">
                    <span>
                      {{ item.title }}
                    </span>
                  </a>
                ...
                <div class="postCon">
                  <div class="c_b_p_desc" id="postlist_description_11168708">
                    ...
                    <a :href="'http://www.inlife.ink/detail?id=' + item.id" class="c_b_p_desc_readmore">阅读全文</a>
                  </div>
                </div>
                <div class="clear"></div>
                <div class="postDesc">
                  <span data-post-id="11168708" class="post-view-count">阅读({{ item.pageView }})</span>
                </div>
                <div class="clear"></div>
              </div>
...
export default class Index extends Vue {
  ...
  async created() {
    await ArticleStore.findAll();
    this.articleList = ArticleStore.articleList;
  }
  ...
  filtArticle(tag: string) {
    this.showTag = tag;
  }

  isShow(tag: string) {
    if (!this.showTag || this.showTag === tag) {
      return true;
    }
    return false;
  }
